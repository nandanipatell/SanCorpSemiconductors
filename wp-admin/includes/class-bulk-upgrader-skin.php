...
class Bulk_Upgrader_Skin extends WP_Upgrader_Skin {
	...
	public function after( $title = '' ) {
		...
		if ( $this->result && ! is_wp_error( $this->result ) ) {
			if ( ! $this->error ) {
				// Check if the success message string has placeholders
				$success_message = $this->upgrader->strings['skin_update_successful'];
		
				// Count the number of placeholders in the string
				$placeholder_count = substr_count($success_message, '%s');
		
				// Ensure the correct number of arguments for sprintf()
				if ($placeholder_count === 1) {
					$formatted_message = sprintf($success_message, $title);
				} elseif ($placeholder_count === 2) {
					$formatted_message = sprintf($success_message, $title, ''); // Add a second argument if needed
				} else {
					$formatted_message = $success_message; // Fallback in case of mismatch
				}
		
				echo '<div class="updated js-update-details" data-update-details="progress-' . esc_attr( $this->upgrader->update_current ) . '">' .
					'<p>' . $formatted_message .
					' <button type="button" class="hide-if-no-js button-link js-update-details-toggle" aria-expanded="false">' . __( 'More details.' ) . 
